      ...
      if not DB_AVAILABLE:
            ...
            try:
                      return mysql.connector.connect(
                                    ...
                                    connection_timeout=5, charset="utf8",
                      )
...
def log_actuator_event(actuator, state, trigger_src=None, duration_sec=None) -> bool:
      conn = get_connection()
    if not conn:
              return False
          try:
...
def log_timelapse_image(filename, image_type="lowres", resolution="640x480",
                                                nas_path=None, synced=False) -> bool:
                                                      conn = get_connection()
                                                      if not conn:
                                                                return False
                                                            try:
...
def log_timelapse_video(filename, nas_path=None,
                                                period_from=None, period_to=None, frame_count=None) -> bool:
                                                      conn = get_connection()
    if not conn:
              return False
    try:
...
def build_sample_data_from_db(actuator_states=None, system_info=None) -> dict:
      """
          Bygg sample_data.json fran MariaDB.
              Ersatter CSV-baserad metod i cloud_sync.py.
                  """
    ...
    return {
              ...
              "social_media": {
                            "enabled": True, "platform": "X / Twitter", "handle": "@lemkil76",
                            "recent_posts": [
                                              {
              ...
              "daily_summary": {
                            ...
                            "images_captured_lowres": 0, "images_captured_hires": 0,
